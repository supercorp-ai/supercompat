...
function GroupRow({ state, now }: { state: GroupState; now: number }) {
  ...
  if (state.status === 'done') {
    const icon = state.fail > 0 ? '❌' : state.pass === 0 ? '⚠️ ' : '✅'
    const iconColor = state.fail > 0 ? 'red' : state.pass === 0 ? 'yellow' : 'green'
    return (
      <Box>
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const runGroup = (group: GroupConfig, index: number) => {
      const cmd = `${baseCmd} --test-concurrency=${group.concurrency} ${group.pattern}`
      // Limit Prisma connection pool to 2 per group to avoid exhausting PostgreSQL max_connections
      ...
      return new Promise<void>((resolve) => {
          ...
          setStates(prev => {
            ...
            for (const line of text.split('\n')) {
              ...
              if (fm) {
                s.fail++
                s.currentTest = fm[1]
                s.failures.push(fm[1])
                log(`FAIL [${group.name}] ${fm[1]}`)
                logError(`FAIL [${group.name}] ${fm[1]}`)
              }
              const tm = line.match(/ℹ tests (\d+)/)
              if (tm) s.totalTests = parseInt(tm[1])
            }
            next[index] = s
            return next
          })
        ...
        child.on('close', () => {
          ...
          // Write failures and stderr to error log
          const hasFailures = /✖/.test(stdout)
          if (hasFailures || stderr.trim()) {
            logError(`\n${'='.repeat(60)}`)
            logError(`FAIL [${group.name}]`)
            logError(`${'='.repeat(60)}`)
            if (hasFailures) {
              // Extract failed test output blocks from stdout
              logError(stdout)
            }
            if (stderr.trim()) {
              logError(`\n--- stderr ---`)
          ...
          setStates(prev => {
            ...
            if (sm) s.skip = parseInt(sm[1])
            const tm = stdout.match(/ℹ tests (\d+)/)
            if (tm) s.totalTests = parseInt(tm[1])
            next[index] = s
            return next
          })
  ...
  useEffect(() => {
    ...
    log(`\n${'='.repeat(60)}`)
    log(`FINAL: ${totalPass} pass, ${totalFail} fail, ${totalSkip} skip`)
  ...
  const allFailures = states.flatMap(s => s.failures.map(f => ({ group: s.name, test: f })))
