...
async function runComputerUseLoop({
  ...
  while (run.status === 'requires_action' && iterations < maxIterations) {
    ...
    for (const tc of toolCalls) {
      ...
      if (action.type === 'click') {
        allCoords.push(`(${action.x},${action.y})`)
      }
  ...
  const textMessage = assistantMessages.find((m: any) => {
...
test('openRouter Kimi K2.5: click Subscribe button on supercorp.ai and describe modal fields', { timeout: 300_000 }, async () => {
  ...
  // Reset browser state to supercorp.ai (the defaultUrl)
  await mcpClient.callTool('computer_call', { action: { type: 'keypress', keys: ['Escape'] } })
  await mcpClient.callTool('computer_call', { action: { type: 'keypress', keys: ['F5'] } })
  ...
  const { run, allActions, allCoords, text } = await runComputerUseLoop({
    client,
    prisma,
    mcpClient,
    instructions: SYSTEM_INSTRUCTIONS,
    ...
    maxIterations: 15,
    testLabel: 'Kimi Subscribe',
  })

  assert.notEqual(run.status, 'failed', `Run should not fail (was: ${JSON.stringify(run.last_error)})`)
  assert.equal(run.status, 'completed', `Expected completed, got ${run.status}`)

  // Model must have clicked at least once (the subscribe button)
