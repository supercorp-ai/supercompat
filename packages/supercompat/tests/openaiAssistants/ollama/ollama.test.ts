/**
 * Smoke suite for the Ollama adapter — mirrors tests/openaiAssistants/openRouter/openRouter.test.ts.
 *
 * Covers the same six surface areas: non-stream chat, streaming chat, direct
 * tool-calling, thread/run via completionsRunAdapter, thread/run + tool
 * calling, and models list. Uses a local Ollama server (OLLAMA_BASE_URL).
 */
...
const MODEL = await skipIfNoModel()

function makeOllama() {
  return new OpenAI({ apiKey: 'ollama', baseURL: ollamaBaseUrl })
}

describe('tests', { concurrency: true, timeout: 360_000 }, () => {
  // =========================================================================
  // 1. Basic chat completion (non-streaming)
  // =========================================================================
  test('ollama: basic chat completion', { timeout: 300_000 }, async () => {
    ...
    const result = await client.chat.completions.create({
      model: MODEL,
      messages: [
        { role: 'user', content: 'What is 2 + 2? Reply with just one number and nothing else.' },
      ],
    })

    const choices = 'choices' in result ? result.choices : (result as any).data.choices
    const message = choices[0]?.message?.content?.trim()
    assert.ok(message?.includes('4'), `Expected "4" in "${message}"`)
  })

  // =========================================================================
  // 2. Streaming chat completion
  // =========================================================================
  test('ollama: streaming chat completion', { timeout: 300_000 }, async () => {
    const client = supercompat({
      clientAdapter: ollamaClientAdapter({ ollama: makeOllama() }),
    })
  ...
  // =========================================================================
  test('ollama: tool calling', { timeout: 300_000 }, async () => {
    const client = supercompat({
      clientAdapter: ollamaClientAdapter({ ollama: makeOllama() }),
    })
    ...
    const result = await client.chat.completions.create({
      model: MODEL,
      messages: [
        { role: 'system', content: 'You are a billing assistant. You can ONLY look up information by calling functions. You have no data yourself.' },
        { role: 'user', content: 'What is the account balance for user ID usr_abc123?' },
      ],
      tools,
    })
  ...
  // =========================================================================
  test('ollama: thread/run via completionsRunAdapter', { timeout: 360_000 }, async () => {
    ...
    await client.beta.threads.runs.createAndPoll(thread.id, { assistant_id: assistant.id })
  ...
  // =========================================================================
  test('ollama: thread/run with tool calling', { timeout: 600_000 }, async () => {
    const prisma = createTestPrisma()
    ...
    let current = await client.beta.threads.runs.submitToolOutputsAndPoll(run.id, {
      thread_id: thread.id,
    ...
    for (let i = 0; i < 5 && current.status === 'requires_action'; i++) {
      ...
      current = await client.beta.threads.runs.submitToolOutputsAndPoll(current.id, {
        thread_id: thread.id,
  ...
  // 6. Models list
  // =========================================================================
  test('ollama: models list', { timeout: 30_000 }, async () => {
    const client = supercompat({
      clientAdapter: ollamaClientAdapter({ ollama: makeOllama() }),
    })
    ...
    assert.ok(models.length > 0, 'Expected at least one model pulled in local Ollama')
  })
})
