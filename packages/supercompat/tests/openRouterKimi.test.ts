...
test.skip('openRouter Kimi: thread/run with tool calling', async () => {
  ...
  const assistant = await client.beta.assistants.create({
    model: MODEL,
    instructions: 'You are a billing database lookup assistant. You have ZERO knowledge of any account data. You MUST call the get_account_balance function for EVERY query. NEVER respond with text — ONLY call functions.',
    tools,
  })
  ...
  await client.beta.threads.messages.create(thread.id, {
    role: 'user',
    content: 'Call get_account_balance to look up the balance for user ID usr_abc123.',
  })
